<?php 
    ...
    curl_setopt_array($curl, array(
      ...
      CURLOPT_HTTPHEADER => array(
		...
		header("Location: productos_html.php");
    ...
    curl_setopt_array($curl, array(
      ...
      CURLOPT_HTTPHEADER => array(
		...
		curl_close($curl);
		$tipo_producto = json_decode($response, true);	
	}
?>
<!DOCTYPE html>
<html lang="en">
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Modificar Productos "<?= $data["Detalles"]["0"]['prod_nombre'] ?>"</h1>
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <input type="hidden" name="prod_id" value="<?= $data["Detalles"]["0"]['prod_id'] ?>">
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="prod_nombre" class="form-control" value="<?= $data["Detalles"]["0"]['prod_nombre'] ?>">
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleInputPassword1"  class="form-label">Descripción</label>
                                        <input type="text" name="prod_descripcion" class="form-control" value="<?= $data["Detalles"]["0"]['prod_descripcion'] ?>">
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleInputPassword1" class="form-label">Precio</label>
                                        <input type="text" name="prod_precio"  class="form-control" value="<?= $data["Detalles"]["0"]['prod_precio'] ?>">
                                    </div>

                                    <div class="mb-3">
                                        <label for="exampleInputPassword1" class="form-label">Número de Unidades</label>
                                        <input type="text" name="prod_unidad"  class="form-control" value="<?= $data["Detalles"]["0"]['prod_unidad'] ?>">           
                                    </div>                                

                                    <div class="mb-3">
                                    <label for="exampleInputPassword1" class="form-label">Seleccione el Tipo de Producto</label>
                                      <select name="tipr_id" class="form-select form-select-sm" aria-label=".form-select-sm example" >
                                        <?php foreach($tipo_producto["Detalles"] as $tipo_producto):?>	
                                        <option type="text" value="<?=$tipo_producto["tipr_id"]?>"><?= $tipo_producto["tipr_tipo"] ?></option>
                                        <?php endforeach?>
                                      </select>
                                    </div>

                                    <button type="submit" class="btn btn-success">Actualizar</button>
                                    <a href="productos_html.php" class="btn btn-danger">Cancelar</a>
                                </form>
                            </div>
                        </div>
                    </div>
