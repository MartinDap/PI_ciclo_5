<?php 
	if($_SERVER["REQUEST_METHOD"]=="POST"){
		...
		CURLOPT_URL => 'https://cevicherias.informaticapp.com/sucursal/'.$_POST['sucu_id'],
		...
		CURLOPT_POSTFIELDS => 
			'sucu_nombre='.$_POST["sucu_nombre"].
			'&sucu_direccion='.$_POST["sucu_direccion"].
			'&sucu_telefono='.$_POST["sucu_telefono"].
			'&sucu_departamento='.$_POST["sucu_departamento"].
			'&sucu_provincia='.$_POST["sucu_provincia"].
            ...
            <div id="layoutSidenav_content">
                <main>
                    <div class="container-fluid px-4">
                        <h1 class="mt-4">Modificar Sucursal "<?= $data["Detalle"][0]['sucu_nombre'] ?>"</h1>
                        <div class="card mb-4">
                            <div class="card-body">

                                <form method="post">
								<div class="mb-3">
                                        <label for="exampleInputEmail1" class="form-label">Nombre de la sucursal</label>
										<input type="hidden" name="sucu_id" value="<?= $data["Detalle"][0]['sucu_id'] ?>" class="form-control" aria-describedby="emailHelp">
                                        <input type="text" name="sucu_nombre" value="<?= $data["Detalle"][0]['sucu_nombre'] ?>" class="form-control" aria-describedby="emailHelp">
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleInputPassword1"  class="form-label">Dirección</label>
                                        <input type="text" name="sucu_direccion" value="<?= $data["Detalle"][0]['sucu_direccion'] ?>" class="form-control">
                                    </div>
                                    <div class="mb-3">
                                        <label for="exampleInputPassword1" class="form-label">Telefono</label>
                                        <input type="text" name="sucu_telefono" value="<?= $data["Detalle"][0]['sucu_telefono'] ?>" class="form-control">
                                    </div>
									<div class="mb-3">
                                        <label for="exampleInputEmail1" class="form-label">Departamento</label>
                                        <input type="text" name="sucu_departamento" value="<?= $data["Detalle"][0]['sucu_departamento'] ?>" class="form-control" aria-describedby="emailHelp">
                                    </div>
									<div class="mb-3">
                                        <label for="exampleInputEmail1" class="form-label">Provincia</label>
                                        <input type="text" name="sucu_provincia"value="<?= $data["Detalle"][0]['sucu_provincia'] ?>" class="form-control" aria-describedby="emailHelp">
                                    </div>
									<div class="mb-3">
                                    ...
                                    <a href="sucursal_html.php" class="btn btn-danger">Cancelar</a>
                                </form>
                            </div>
                        </div>
                    </div>
